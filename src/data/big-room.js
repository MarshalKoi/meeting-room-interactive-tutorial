export const bigRoomCards = [
  {
    slug: "trinh-chieu-co-day",
    title: "Trình chiếu có dây",
    desc: "Kết nối laptop qua HDMI",
    icon: "Cable",
    steps: [
      "Bật PC trong phòng họp bằng **nút nguồn** ở mặt dưới góc bên phải máy và đợi 5 phút.",
      "Lấy **cáp HDMI** đặt trong **tủ phía ngoài cùng bên trái**.",
      "Cắm cáp HDMI vào cổng trong **hộp âm bàn** trên bàn họp.",
      "Trên touch panel TC10, chọn **'Phòng họp lớn'** trên màn hình chính.",
      "Chọn tab **'Kịch bản AV'** -> **'Trình chiếu có dây'** -> **'ÁP DỤNG'**.",
      "Kiểm tra lại xem tiếng của laptop đã ra FDX chưa.",
    ], 
  },
  {
    slug: "trinh-chieu-khong-day",
    title: "Trình chiếu không dây",
    desc: "Kết nối laptop qua Via Go3 hoặc Poly G62",
    icon: "Cast",
    steps: [
      "Bật PC trong phòng họp bằng **nút nguồn** ở mặt dưới góc bên phải máy và đợi 5 phút.",
      "Trên touch panel TC10, chọn **'Phòng họp lớn'** trên màn hình chính.",
      "Chọn tab **'Kịch bản AV'** -> **'Trình chiếu không dây'** 1 hoặc 2. -> **'ÁP DỤNG'**.",
      "Kết nối vào wifi phòng họp bằng QR hoặc tự nhập: **AVI_Lab/Av@2025a**",
      "Không dây 1 dùng **Via Go3**, Không dây 2 dùng **Poly G62**.",
      "Đối với Via Go3, chọn kịch bản **'Trình chiếu không dây 1'**.",
      "**Lưu ý: Nếu trên màn hình hiển thị dòng chữ 'loading linux...' cần đợi một khoảng 10 phút hoặc khởi động lại thiết bị ViaGo3**.",
      "Truy cập địa chỉ **10.10.1.22** và chọn kết nối bằng web hoặc tải app về và kết nối bằng app.",
      "Nhập **code** hiển thị trên TV và bắt đầu chia sẻ màn hình.",
      "Đối với Poly G62, chọn kịch bản **'Trình chiếu không dây 2'**.",
      "Trên PC click chuột phải chọn **'Display Settings'** và chọn **'Connect to a wireless display'**.",
      "Chọn **'AVIJSC-67VC'** từ danh sách thiết bị và nhập code hiển thị trên TV",
      "Người dùng **Mac/iPhone** có thể dùng **AirPlay**.",
    ],
  },
  {
    slug: "hop-video",
    title: "Họp Teams/Zoom/Google Meet",
    desc: "Sử dụng camera, micro và loa phòng họp",
    icon: "Webcam",
    steps: [
      "Bật PC trong phòng họp bằng **nút nguồn** ở mặt dưới góc bên phải máy và đợi 5 phút.",
      "Trên touch panel TC10, chọn **'Phòng họp lớn'** -> **'Họp trực tuyến'** -> **'BYOM'** -> **'Áp dụng'**.",
      "Vào mục camera và call **preset 0** của camera 1",
      "Trên touch panel TC10 trên mặt bàn họp, chọn **'Device Mode'**.",
      "Rút ra và cắm lại **dây usb màu trắng** phía bên phải PC để máy nhận thiết bị âm thanh và camera của phòng họp.",
      "Mở Teams/Zoom/Google Meet trên PC.",
      "Kết nối với ViaGo3 để trình chiếu cuộc họp",
      "Chọn camera, mic và loa của phòng họp tên là **'Poly Studio G62'**",
    ],
  },
  {
    slug: "am-thanh",
    title: "Điều khiển âm thanh",
    desc: "Tăng giảm âm lượng",
    icon: "Volume2",
    steps: [
      "Trên touch panel TC10, chọn **'Phòng họp lớn'**",
      "**Master volume**: điều chỉnh trên touch panel, khuyến nghị giữ nguyên",
      "Nhấn **Tắt tiếng/Bật tiếng** trên touch panel để mute/unmute âm lượng tổng.",
      "Nếu âm thanh vọng hãy **giảm volume**",
    ],
  },
  {
    slug: "camera-preset",
    title: "Camera & Preset",
    desc: "Điều khiển camera PTZ và preset vị trí",
    icon: "Camera",
    steps: [
      "Trên touch panel, mở tab **'Điều khiển Camera'** để xem danh sách camera PTZ trong phòng.",
      "Chọn camera muốn điều khiển.",
      "Điều khiển thủ công: dùng nút **Pan/Tilt** (lên/xuống/trái/phải) và **Zoom +/−** trên panel.",
      "Lưu preset mới: chỉnh camera về vị trí mong muốn, gõ số preset và nhấn **'Save'** hoặc **'Lưu'**.",
    ],
  },
  {
    slug: "ho-tro",
    title: "Gọi hỗ trợ kỹ thuật",
    desc: "Liên hệ bộ phận AV/IT khi cần hỗ trợ",
    icon: "Phone",
    steps: [
      "Hotline nội bộ: gọi số **0853134368** (phục vụ giờ hành chính **8:00–17:30**, thứ Hai–Sáu).",
      "Ngoài giờ hành chính: nhắn tin qua **Zalo**.",
    ],
  },
];
